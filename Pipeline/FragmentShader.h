...
struct FragmentPayLoad {
  ...
  Vec3 position; // vị trí của điểm ảnh trong không gian thế giới
  FragmentPayLoad(const Vec3 &color = Vec3(1.0f, 1.0f, 1.0f),
                  const Vec3 &normal = Vec3(0.0f, 0.0f, 1.0f),
                  const float &depth = 1.0f,
                  const Vec3 &position = Vec3(0.0f, 0.0f, 0.0f))
      : color(color), normal(normal), depth(depth), position(position) {}
};
...
public:
  ...
  char shade() const {
    ...
    Vec3 radianceD = distantLight.getRadiance(fragmentPayLoad.position);
    Vec3 diffuse = radianceD * lambertian;

    Vec3 ambient = ambientLight.getRadiance(fragmentPayLoad.position);
